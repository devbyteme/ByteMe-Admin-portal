...
const GeneralAdminLogin = () => {
  ...
  const { isAuthenticated, isGeneralAdmin } = useAuth();

  // Redirect if already authenticated as general admin
  if (isAuthenticated && isGeneralAdmin) {
    return <Navigate to="/dashboard" replace />;
  }

  // Redirect if authenticated as multi-vendor admin
  if (isAuthenticated && !isGeneralAdmin) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        if (result.token) {
          localStorage.setItem('generalAdminToken', result.token);
        }
        if (result.user) {
          localStorage.setItem('generalAdminUser', JSON.stringify(result.user));
        }
